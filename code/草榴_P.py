...
def _1_get_url(page,pic_type):
    if pic_type == '1':
        url = 'https://cl.7635x.xyz/thread0806.php?fid=7&search=&page=' + page  # 技术交流
    if pic_type == '2':
        url = 'https://cl.7635x.xyz/thread0806.php?fid=8&search=&page=' + page  # 新時代的我們
    if pic_type == '3':
        url = 'https://cl.7635x.xyz/thread0806.php?fid=16&search=&page=' + page  # 達蓋爾的旗幟

    # url = 'https://cl.7635x.xyz/@%E5%9B%9E%E6%94%B6%E8%A1%A8%E5%A6%B9' #注意看好url
            ...
            if i not in crawled:
                try:
                    with open('已爬取草榴p.log', 'a') as file:
                        file.write(i+'\n')
                    imgs = getimgscl('https://cl.7635x.xyz/' + i + 'html')
                    title = imgs[0]
                    print(title)
                    del imgs[0]
